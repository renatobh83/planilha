	...
	const formWinLoss = (e) =>{
		...
		if(win) {
			...
			if(nivelSoros === 0 && nunGale === 0 ){
				...
				setTotalLoss(0)
				setTotalPrejuizo(0)
				setLucroAcumulado((parseFloat(soros) - parseFloat(entradaInicial)).toFixed(2))
			} 
				...
				setTotalLoss(0)
				setTotalPrejuizo(0)
				setLucroAcumulado((parseFloat(soros2) + parseFloat(lucroAcumulado)).toFixed(2))
			}
				...
				setTotalLoss(0)
				setTotalPrejuizo(0)
				setLucroAcumulado((parseFloat(soros3) + parseFloat(lucroAcumulado)).toFixed(2))
			}
			else if(nivelSoros === 3) {
				setBtnWin(true)
				setBtnLoss(true)
				setTotalPrejuizo(0)
				setLucroAcumulado((parseFloat(lucroN3) + parseFloat(lucroAcumulado)).toFixed(2))
				setValorBanca((parseFloat(capital)+parseFloat(lucroN3) + parseFloat(lucroAcumulado)).toFixed(2))
			}
		}
		else if(loss) {
			if(nivelSoros === 0 && totalLoss === 0 ) {
				...
				setBtnWin(true)
				setBtnLoss(true)
				setTotalPrejuizo((parseFloat(entradaInicial)+parseFloat(gale1)).toFixed(2))
			}
				...
				setNivelSoros(1)
				setTotalLoss(totalLoss+ 1)
				setLucroAcumulado((parseFloat(lucroAcumulado)-parseFloat(soros2)).toFixed(2))
			}
			else if(nivelSoros === 3){
				setEntrada(soros2)
				setNivelSoros(2)
				setTotalLoss(totalLoss + 1)
				setBtnWin(false)
				setLucroAcumulado((parseFloat(lucroAcumulado)-parseFloat(soros3)).toFixed(2))
        ...
        if (!/[0-9 , .]/.test(event.key)) {
          event.preventDefault();
			...
			<form onSubmit={formWinLoss}>
				<button className="win" onClick={()=>setWin(true)} disabled={btnWin}> Win </button>
				<button className="loss" onClick={()=>setLoss(true)} disabled={btnLoss}> Loss </button>
			</form>
				)}
			<div className="valores">
			<h4>{nivelSoros} x {totalLoss}</h4>
